            ...
            Ok(data) => {
                ...
                if data.len() > 0 {
                    for addr_db in data.split(",") {
                        // 检查玩家ip地址健康检查状态
                        if let Ok(health) =
                            game_db::find(GameData::player_addr_health(addr_db.to_string(), None))
                        {
                            ...
                            if let Ok(h) = health.parse::<u128>() {
                                ...
                                if now > (h + clean_tick) {
                                    // println!("now: {}", now);
                                    if let Ok(uid) = game_db::find(GameData::player_addr_uid(
                                        addr_db.to_string(),
                                        None,
                                    )) {
                                        // println!("uid: {}", uid);
                ...
                match packet {
                    Packet::Heartbeat(heartbeat_route) => match heartbeat_route {
                        ...
                        protocol::route::HeartbeatRoute::Keep(_) => {
                            ...
                            // 添加玩家ip地址健康检查状态
                            let _ = game_db::save(GameData::player_addr_health(
                                addr.to_string(),
                                Some(format!(
                                    "{}",
                                    SystemTime::now()
                            ...
                            match game_db::find(GameData::player_addr_uid(addr.to_string(), None)) {
                                Ok(data) => {
                                    if let Ok(id) = data.parse::<u32>() {
                                        ...
                                        if let Ok(id) =
                                            game_db::next_u64(GameData::player_queue_uid(None))
                                        {
                                            uid = id as u32;
                                            let _ = game_db::save(GameData::player_addr_uid(
                                                addr.to_string(),
                                                Some(uid.to_string()),
                                            ));
                                        }
